...
function Resolve-ContentRoot {
    ...
    if ($dirsA.Count -gt 0) { return $candidateA }
    ...
    if (Test-Path $candidateB) {
        ...
        if ($dirsB.Count -gt 0) { return $candidateB }
    }
...
$defaultMetadataFile = Join-Path $repoRoot '.github\skills\ebook-build\configs\amazon-kdp-guide.metadata.yaml'
...
Ensure-Path -Path (Join-Path $KindleTemplateDir 'add-pagelist-functions.ps1') -Label 'add-pagelist-functions.ps1'
...
$chapterFiles = @(
    foreach ($d in $chapterDirs) {
        Get-ChildItem -Path $d.FullName -File -Filter '*.md' | Where-Object { $_.Name -match $ChapterFilePattern }
    }
)
...
foreach ($dir in $chapterDirs) {
    Copy-Item -Path $dir.FullName -Destination (Join-Path $stageBookRoot $dir.Name) -Recurse -Force
}
...
Copy-Item -Path (Join-Path $KindleTemplateDir 'add-pagelist-functions.ps1') -Destination (Join-Path $stageKindle 'add-pagelist-functions.ps1') -Force
...
foreach ($fmt in $Formats) {
    $produced = Get-ChildItem -Path $stageOutput -Filter "*.$fmt" -File -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($null -eq $produced) {
        Write-Warning "Requested format not found: $fmt"
        continue
    }
    $destPath = Join-Path $OutputDir ("$ProjectName.$fmt")
