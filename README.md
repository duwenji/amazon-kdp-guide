...
② 02-account-setup.md（アカウント作成）
  ↓
③ 01-prepare-manuscript.md（原稿準備）
  ↓
④ 02-upload-publish.md（出版）
  ↓
...
この教材では、以下の 2 つのリポジトリを利用します。

| 種別 | リポジトリ | 役割 | GitHub |
|---|---|---|---|
| 利用側リポジトリ（consumer repo） | `amazon-kdp-guide` | 原稿・設定ファイル・実行用 wrapper を持つ利用側リポジトリ | `https://github.com/duwenji/amazon-kdp-guide` |
| 共有リポジトリ（shared repo） | `shared-copilot-skills` | `ebook-build` の共通実装を提供する共有リポジトリ | `https://github.com/duwenji/shared-copilot-skills` |

### 用語の定義

- **利用側リポジトリ（consumer repo）**: 実際に原稿・設定ファイル・成果物を持つ作業対象のリポジトリ
- **共有リポジトリ（shared repo）**: 共通 Skill やスクリプト実装を提供するリポジトリ
- **実行用 wrapper（利用者向け入口）**: `invoke-build.ps1`。利用者が直接実行するスクリプト
- **共有実装**: `invoke-ebook-build.ps1` や `convert-to-kindle.ps1` など、wrapper の内部で呼ばれる処理

ローカル作業フォルダ:
- `c:\dev\apps\amazon-kdp-guide`
...
- PDF 生成時は `Chrome` または `Edge` をインストール
...
このリポジトリでは `shared-copilot-skills` を **共有リポジトリ（shared repo）** として利用します。
submodule 方式はリポジトリ単位の導入になるため、個別スキルのみを選択して導入することはできません。

- Ebook build の実行用 wrapper: `./.github/skills-config/ebook-build/invoke-build.ps1`
...
| # | 特徴 | 説明 |
|----|------|------|
| 1️⃣ | **登録コストが低い** | KDP の登録・アップロード自体は無料 |
| 2️⃣ | **高い印税率** | 70% まで獲得（出版社経由より高い） |
| 3️⃣ | **比較的早く販売開始** | 審査後、通常は数時間～48時間程度で販売開始 |
| 4️⃣ | **世界規模** | 170+ の国と地域で販売 |
| 5️⃣ | **ISBN 対応** | Kindle 電子書籍は通常 ISBN 不要。紙書籍では無料 ISBN を選べる場合あり |
...
A：**KDP の登録・ファイルアップロード自体は無料**で始められます。ただし、原稿制作・表紙作成・外注・広告などは別途費用が発生する場合があります。売上に応じて 35～70% のロイヤリティを獲得できます。
...
3. 01-prepare-manuscript.md で EPUB / PDF / KDP 登録情報を生成（30 分）
4. 02-upload-publish.md に従って出版申請を進める（30 分）
→ 合計 95 分で出版申請まで進めます
```
