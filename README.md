...
④ 04-upload-publish.md（出版）
  ↓
⑤ 参考資料: 05-metadata-and-pricing.md
  ↓
✅ 販売開始！
```
...
```
上記のロードマップ + ⑥ 06-kindle-unlimited.md
```
...
入口ファイル:
- [ebook-build Skill 定義](c:\dev\apps\amazon-kdp-guide\.github\skills\ebook-build\SKILL.md)
- [ebook-build 実行ガイド](c:\dev\apps\amazon-kdp-guide\.github\skills\ebook-build\docs\README.md)
- [ebook-build 実行スクリプト](c:\dev\apps\amazon-kdp-guide\.github\skills\ebook-build\scripts\invoke-ebook-build.ps1)
...
出力先:
- [ebook-output](c:\dev\apps\amazon-kdp-guide\ebook-output)

### **clean-architecture を Amazon KDP で販売する最短フロー**

```
ステップ 1️⃣: 原稿を準備
  → c:\dev\apps\clean-architecture\kindle\
  → convert-to-kindle.ps1 を実行
  → output/clean-architecture.azw3 を取得
...
```
c:\dev\apps\clean-architecture\kindle\
  ├── convert-to-kindle.ps1    ← AZW3 生成
  ├── metadata.yaml            ← メタデータ定義
  ├── style.css                ← スタイル設定
  └── output/
      └── clean-architecture.azw3  ← KDP で使用
...
```
1. c:\dev\apps\clean-architecture\kindle\README.md で概要確認
   ↓
2. convert-to-kindle.ps1 で AZW3 を生成
   ↓
3. このガイドで出版方法を学習
   ↓
...
### **05-metadata-and-pricing.md を読んだ後**
- [ ] 効果的なキーワード選択ができた
- [ ] 適切な価格を設定できた
- [ ] SEO を考慮した説明文が書ける

### **06-kindle-unlimited.md を読んだ後**
- [ ] Kindle Unlimited の仕組みが理解できた
- [ ] 参加すべきか判断できた
- [ ] 読放題での収入を計算できた

### **07-marketing-tips.md を読んだ後**
- [ ] SNS での宣伝方法がわかった
- [ ] レビュー獲得のコツを知った
- [ ] セール戦略を立てられた

### **08-troubleshooting-faq.md を読んだ後**
- [ ] よくある問題が解決できた
- [ ] トラブル時の対応がわかった
- [ ] 自信を持って販売開始できた
...
**最後の確認事項:**
- [ ] c:\dev\apps\clean-architecture\kindle\README.md を確認した
- [ ] AZW3 ファイル生成方法を理解した
- [ ] このガイドの概要を読んだ
